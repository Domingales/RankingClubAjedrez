(function () {
  ...
  function renderExternalGames(){
    ...
    el.innerHTML=`<table><thead><tr><th>Fecha</th><th>Blancas</th><th>Resultado</th><th>Negras</th><th>Ritmo</th><th>Evento / lugar</th><th>PGN</th><th></th></tr></thead><tbody>${games.map(g=>`<tr><td>${fmtDate(asDate(g.playedAt))}</td><td>${esc(g.whiteName)}</td><td><strong>${esc(g.result||'*')}</strong></td><td>${esc(g.blackName)}</td><td>${esc(g.timeControl||'')}</td><td>${esc(g.event||'—')}</td><td>${pgnButton(g,'PGN')}</td><td><button class="danger-outline small" onclick="ClubApp.deleteExternalGame('${g.id}')">Eliminar</button></td></tr>`).join('')}</tbody></table>`;
  }
