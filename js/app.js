(function () {
  ...
  function refreshAll() {
    if (!state.me) return;
    populateSelectors();
    renderDashboard(); renderPending(); renderGames(); renderProfile(); renderPlayers(); renderH2H(); renderRecords(); renderAdmin(); renderGameDraftBanner();
  ...
  function gameRow(g) {
    ...
    return `<div class="game-row"><div class="game-main"><strong>${esc(playerName(g.whiteUid))} ${gameResultText(g)} ${esc(playerName(g.blackUid))}</strong><div class="game-meta">${d?fmtDate(d):''} · ${esc(g.timeControl||'')} ${g.event?'· '+esc(g.event):''}</div></div>${hasPgn(g)?pgnButton(g,'PGN'):''}<span class="result-pill draw">${gameResultText(g)}</span></div>`;
  }
  ...
  function myDraftGame() {
    ...
    return state.games.find(g => g.status === 'draft' && g.creatorUid === state.authUser.uid) || null;
  }
  ...
  function gameAnnotationMeta(g) {
    return {
      ...
      whiteElo: g.whiteRatingAtStart ?? userById(g.whiteUid)?.rating,
      blackElo: g.blackRatingAtStart ?? userById(g.blackUid)?.rating,
  ...
  async function startAnnotation() {
    ...
    try {
      const ref = await db.collection('games').add(draftData);
      ...
      annotationState.orientation = form.myColor;
  ...
  async function getGameById(id) {
    const local = state.games.find(g => g.id === id);
  ...
  async function resumeDraft(id) {
    try {
      ...
      if (!g || g.status !== 'draft' || g.creatorUid !== state.authUser.uid) return showMessage('Ese borrador ya no está disponible.','error');
      const engine = new window.ClubChessRecorder();
      ...
      annotationState.orientation = g.whiteUid === state.authUser.uid ? 'white' : 'black';
  ...
  function populateFormFromDraft(g) {
    ...
    $('gameMyColor').value = g.whiteUid === state.authUser.uid ? 'white' : 'black';
  ...
  async function saveAnnotationDraft(showSuccess) {
    ...
    try{
      ...
      await db.collection('games').doc(annotationState.gameId).update({
  ...
  async function sendAnnotatedGame() {
    ...
    try{
      ...
      const myColor=g.whiteUid===state.authUser.uid?'white':'black';
      const result=resultFromMyPerspective($('annotationResult').value,myColor);
      const moves=annotationState.engine.history();
      const pgn=annotationState.engine.pgn(gameAnnotationMeta(g),result);
      await db.collection('games').doc(g.id).update({
        status:'pending',result,annotationFinished:true,moves,moveCount:moves.length,
        currentFen:annotationState.engine.fen(),finalFen:annotationState.engine.fen(),pgn,
        updatedAt:firebase.firestore.FieldValue.serverTimestamp()
      });
      resetAnnotationUI();
      $('gameEvent').value='';$('gameLink').value='';
      showMessage('Partida guardada con su PGN. El rival debe confirmarla.','success');
      showView('pending');
    }catch(err){console.error(err);showMessage(friendlyError(err),'error');}
  }
  ...
  async function discardDraft(id,fromActive) {
    ...
    try{
      const g=await getGameById(id);
      if(!g||g.status!=='draft'||g.creatorUid!==state.authUser.uid)throw new Error('No puedes borrar este borrador.');
  ...
  function openPgn(id) {
    const g=state.games.find(x=>x.id===id); if(!g||!hasPgn(g))return showMessage('Esta partida no tiene PGN guardado.','error');
    const moves=Array.isArray(g.moves)?g.moves:[];
    pgnViewerState.gameId=id;pgnViewerState.index=0;pgnViewerState.fens=[window.ClubChessRecorder.START_FEN,...moves.map(m=>m.fen).filter(Boolean)];
    openModal(`<div class="pgn-viewer"><h2>${esc(playerName(g.whiteUid))} ${gameResultText(g)} ${esc(playerName(g.blackUid))}</h2><div id="pgnViewerBoard" class="pgn-viewer-board"></div><div id="pgnViewerMeta" class="pgn-meta"></div><div class="pgn-controls"><button class="outline" type="button" onclick="ClubApp.pgnJump('start')">|◀</button><button class="outline" type="button" onclick="ClubApp.pgnJump('prev')">◀</button><button class="outline" type="button" onclick="ClubApp.pgnJump('next')">▶</button><button class="outline" type="button" onclick="ClubApp.pgnJump('end')">▶|</button></div><textarea id="pgnViewerText" class="pgn-text" readonly>${esc(g.pgn)}</textarea><button class="primary" type="button" onclick="ClubApp.copyPgn('${g.id}')">Copiar PGN</button></div>`);
    renderPgnViewer();
  }
  ...
  function renderPgnViewer(){
    ...
    const g=state.games.find(x=>x.id===pgnViewerState.gameId),move=i>0?(g?.moves||[])[i-1]:null;
  ...
  async function copyPgn(id){
    const g=state.games.find(x=>x.id===id);if(!g||!g.pgn)return;
  ...
  window.ClubApp={confirmGame,rejectGame,cancelGame,playerModal,toggleActive,toggleRole,openEditProfile,resumeDraft,discardDraft,choosePromotion,openPgn,pgnJump,copyPgn};
