(function () {
  ...
  function openEditProfile() {
    ...
    openModal(`
      ...
      <div class="profile-security-box">
        ...
        <p class="small-note">Si quieres cambiar tu contraseña, Firebase enviará un enlace seguro a tu correo de acceso.</p>
        <button id="profilePasswordResetBtn" class="outline" type="button">🔑 Cambiar contraseña</button>
    ...
    $('profilePasswordResetBtn').addEventListener('click', sendProfilePasswordReset);
  ...
  async function sendProfilePasswordReset() {
    const email = state.authUser?.email || state.me?.email || '';
    if (!email) return profileEditMessage('No se ha encontrado el correo de acceso.', 'error');
    const button = $('profilePasswordResetBtn');
    button.disabled = true;
    button.textContent = 'Enviando…';
    try {
      await auth.sendPasswordResetEmail(email);
      profileEditMessage(`Enlace enviado a ${email}. Revisa también la carpeta de correo no deseado.`, 'success');
