(function () {
  ...
  function renderRecords() {
    ...
    const rec=[['👑','Mayor rating actual',best.name,ratingLabel(best)],['🚀','Máximo histórico',peak.name,round(peak.maxRating)],['♟️','Más partidas',games.name,(games.gamesPlayed||0)+' PJ'],['🏆','Más victorias',wins.name,(wins.wins||0)+' victorias'],['🎯','Mejor porcentaje de puntos',pctBest?pctBest.name:'—',pctBest?scorePct(pctBest)+'%':'—'],['⚡','Mayor sorpresa',upset?playerName(upset.winner):'—',upset?`ganó con ${round(upset.diff)} puntos menos`:'—']];
