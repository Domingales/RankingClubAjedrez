(function () {
  ...
  function renderAnnotation() {
    ...
    ranks.forEach((rank,ri)=>files.forEach((file,fi)=>{
      ...
      html+=`<button class="${classes.join(' ')}" type="button" data-square="${sq}" aria-label="${sq}">${p?`<span class="chess-piece ${p.color==='w'?'white':'black'}">${glyph[p.color+p.type]}</span>`:''}${showFile?`<span class="annotation-coord-file">${file}</span>`:''}${showRank?`<span class="annotation-coord-rank">${rank}</span>`:''}</button>`;
    }));
  ...
  function askPromotion(from,to) {
    ...
    openModal(`<h2>Promoción</h2><p>Elige la pieza en la que se convierte el peón.</p><div class="promotion-choices">${['q','r','b','n'].map(x=>`<button type="button" class="promotion-choice" onclick="ClubApp.choosePromotion('${x}')"><span class="chess-piece ${white?'white':'black'}">${pieces[x]}</span></button>`).join('')}</div>`);
  }
  ...
  function hasPgn(g){return Boolean(g&&g.pgn&&String(g.pgn).trim());}
  function pgnButton(g,label='Ver PGN'){
    if(hasPgn(g))return `<button class="pgn-button pgn-available" type="button" title="Ver PGN de la partida" aria-label="Ver PGN de la partida" onclick="ClubApp.openPgn('${g.id}')">${label}</button>`;
    return `<button class="pgn-button pgn-unavailable" type="button" title="Esta partida no tiene PGN registrado" aria-label="Esta partida no tiene PGN registrado" onclick="ClubApp.noPgn()">${label}</button>`;
  }

  function noPgn(){showMessage('Esta partida no tiene PGN registrado.','error');}
  ...
  function renderFenBoard(containerId,fen) {
    ...
    for(let rank=8;rank>=1;rank--)for(const file of 'abcdefgh'){
      ...
      html+=`<div class="annotation-square ${(c.f+c.r)%2===0?'dark':'light'}">${p?`<span class="chess-piece ${p.color==='w'?'white':'black'}">${glyph[p.color+p.type]}</span>`:''}</div>`;
    }
    el.innerHTML=html;
  }
  ...
  function renderProfile() {
    ...
    $('myRecentGames').innerHTML=myGames().slice(0,6).map(g=>{
      const cls=resultClassFor(g,u.id);return `<div class="game-row"><div class="game-main"><strong>${esc(playerName(g.whiteUid))} ${gameResultText(g)} ${esc(playerName(g.blackUid))}</strong><div class="game-meta">${fmtDate(asDate(g.playedAt))}</div></div>${pgnButton(g,'PGN')}<span class="result-pill ${cls}">${cls==='win'?'Victoria':cls==='loss'?'Derrota':'Tablas'}</span></div>`;
  ...
  window.ClubApp={confirmGame,rejectGame,cancelGame,playerModal,toggleActive,toggleRole,openEditProfile,resumeDraft,discardDraft,choosePromotion,openPgn,noPgn,pgnJump,copyPgn,deleteExternalGame};
